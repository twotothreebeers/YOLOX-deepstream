    ...
    std::vector<NvDsInferParseObjectInfo>& objectList)
{
    ...
    for(auto& r : objects) {
        ...
        oinfo.left    = static_cast<float>(r.rect.x);
	    oinfo.top     = static_cast<float>(r.rect.y);
	    oinfo.width   = static_cast<float>(r.rect.width);
	    oinfo.height  = static_cast<float>(r.rect.height);
        /*oinfo.left = static_cast<float>(r.rect.x);
        oinfo.top = static_cast<float>(r.rect.y);
        oinfo.width = static_cast<float>(r.rect.width);
        oinfo.height = static_cast<float>(r.rect.height);*/
